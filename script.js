...
function updateAccountDisplay() {
  ...
  if (currentUser) {
    ...
    hintText.textContent = "Tu es connecté. Tes vidéos sont liées à ton compte. Maximum 3 vidéos conservées.";
  } else {
    creditsLabel.textContent = `${freeTrials} essai${freeTrials > 1 ? "s" : ""} gratuit${freeTrials > 1 ? "s" : ""}`;
  ...
  if (engine === "veed") {
    ...
    if (!currentUser && !isAdminUser()) {
      ...
      hintText.textContent =
        "Maximum 3 vidéos par compte. Télécharge rapidement. En cas d’affluence, le traitement peut être plus long.";
    }
  } else {
    engineRulesText.textContent =
    ...
    if (isAdminUser()) {
      hintText.textContent = "Compte admin actif. Maximum 3 vidéos affichées par utilisateur standard.";
    } else if (currentUser) {
      hintText.textContent = "Maximum 3 vidéos conservées par compte. Télécharge rapidement ta vidéo.";
    } else {
      hintText.textContent = "Connecte-toi pour utiliser ton espace vidéo personnel.";
    }
  }
...
function clearRecoveryUrl() {
  try {
    const cleanUrl = PASSWORD_RESET_REDIRECT_URL;
...
generateBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (hintText) {
      hintText.textContent = "Sync-up traite la vidéo. En cas d’affluence, le traitement peut être plus long.";
    }
