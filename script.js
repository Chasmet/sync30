...
generateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (engine === "veed") {
      ...
      if (!data.videoUrl) {
        throw new Error("Aucune URL vidéo renvoyée par Wav2Lip");
      }

      previewVideo.src = data.videoUrl;
      downloadBtn.href = data.videoUrl;
      downloadBtn.download = "sync30-wav2lip.mp4";
      hintText.textContent =
        "Mode VEED = test Wav2Lip actif. Tu dois voir une vraie synchro labiale.";
    } else {
...
function updateEngineTexts() {
  ...
  if (engine === "kling") {
    ...
    engineDescription.textContent =
      "Kling = moins cher • remplacement audio simple.";
    ...
    engineRulesText.textContent =
      "Mode VEED actif : test Wav2Lip avec 30 secondes affichées et tolérance technique jusqu’à 32 secondes.";
    engineDescription.textContent =
