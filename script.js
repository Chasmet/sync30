...
generateBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (engine === "veed") {
      ...
      hintText.textContent = "Mode VEED = qualité premium. Retour via URL vidéo.";
    } else {
      ...
      hintText.textContent = "Mode Sync-up = lip-sync économique.";
...
function updateEngineTexts() {
  ...
  if (engine === "veed") {
    ...
    engineDescription.textContent = "VEED = qualité premium.";
...
async function deleteVideo(deleteUrl, videoName) {
...
async function loadVideos() {
  ...
  try {
    const res = await fetch(`${SYNCUP_API_URL}/videos`);
    const data = await res.json();

    if (!data.ok || !Array.isArray(data.videos)) {
      throw new Error("Réponse vidéos invalide");
    }
    ...
    if (data.videos.length === 0) {
      videosContainer.innerHTML = `
        <div class="upload-box">
          ...
          <p>Les vidéos Sync-up sauvegardées apparaîtront ici.</p>
        </div>
      `;
      return;
    }
    ...
    data.videos.forEach((item) => {
      ...
      deleteButton.addEventListener("click", async () => {
        const deleteUrl = `${SYNCUP_API_URL}/video/syncup/${encodeURIComponent(item.name)}`;
        await deleteVideo(deleteUrl, item.name);
      });
