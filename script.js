...
function updateEngineTexts() {
  ...
  if (engineSelect?.options?.[0]) {
    engineSelect.options[0].textContent = "Mode rapide (Sync-up, 30 s max)";
  }

  if (engineSelect?.options?.[1]) {
    engineSelect.options[1].textContent = "Mode qualité (VEED, 30 s max)";
  }

  if (engine === "veed") {
    engineRulesText.textContent =
      "Ajoute une vidéo et un audio. VEED : 30 secondes affichées, 32 secondes tolérées maximum.";
    engineDescription.textContent =
      "VEED = meilleure qualité. Tarif indicatif : 4,19 € pour 30 secondes.";
    ...
    engineRulesText.textContent =
      "Ajoute une vidéo et un audio. Sync-up : 30 secondes maximum.";
    engineDescription.textContent =
      "Sync-up = plus économique. Tarif indicatif : 2,19 € pour 30 secondes.";
    videoRulesText.textContent =
      "Formats supportés : MP4, WEBM, MOV, M4V, OGG • Sync-up : 30 secondes maximum.";
    audioRulesText.textContent =
      "Formats supportés : MP3, WAV, M4A, AAC, OGG, WEBM • Sync-up : 30 secondes maximum.";
  }
}
...
function updateAccountDisplay() {
  ...
  if (currentUser) {
    ...
    hintText.textContent =
      engine === "veed"
        ? "Compte personnel actif. VEED utilise ton solde qualité."
        : "Compte personnel actif. Sync-up utilise ton solde rapide.";
    return;
  }
  ...
  hintText.textContent =
    engine === "veed"
      ? "Mode test public actif. VEED utilise le solde public."
      : "Mode test public actif. Sync-up utilise le solde public.";
}
...
function updateEstimate() {
  ...
  estimateModeText.textContent = engine === "veed" ? "VEED" : "Sync-up";
...
function startProgressAnimation(engine) {
  ...
  let value = 0;
  if (progressFill) progressFill.style.width = "0%";
  if (progressText) progressText.textContent = engine === "veed" ? "Préparation VEED..." : "Préparation Sync-up...";
      ...
      if (progressText) {
        if (engine === "veed") {
          progressText.textContent =
            value < 20 ? "Envoi à VEED..." :
            value < 50 ? "Traitement VEED..." :
            "VEED finalise la vidéo...";
        } else {
          progressText.textContent =
            value < 20 ? "Envoi à Sync-up..." :
            value < 50 ? "Job Sync-up lancé..." :
    ...
    syncupPollingInterval = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "succeeded") {
          ...
          hintText.textContent = "Sync-up terminé. Télécharge rapidement la vidéo.";
          resolve();
          return;
        }
...
async function loadVideos() {
  ...
  try {
    ...
    allVideos.forEach((item) => {
      ...
      meta.className = "sub";
      meta.innerHTML = `
        Moteur : ${item.engine === "veed" ? "VEED" : "Sync-up"}<br>
...
generateBtn?.addEventListener("click", async () => {
  ...
  if (!isAdminUser() && Number(balance || 0) < billedSeconds) {
    showWarning(
      `Pas assez de temps disponible pour ${engine === "veed" ? "VEED" : "Sync-up"}. Requis : ${billedSeconds} s. Disponible : ${Number(balance || 0)} s.`
    );
    return;
  }
  ...
  try {
    ...
    if (engine === "veed") {
      ...
      hintText.textContent = "VEED terminé. Télécharge rapidement la vidéo.";
      setGeneratingState(false);
      return;
    }
    ...
    if (!data.jobId) {
      throw new Error("Job Sync-up non lancé");
    }
