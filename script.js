...
videoInput.addEventListener("change", async () => {
  ...
  if (!isAllowedExtension(file.name, ALLOWED_VIDEO_EXTENSIONS)) {
    ...
    warningText.textContent = "Format vidéo non supporté. Utilise MP4, WEBM, MOV, M4V ou OGG.";
    videoInput.value = "";
...
audioInput.addEventListener("change", async () => {
  ...
  if (!isAllowedExtension(file.name, ALLOWED_AUDIO_EXTENSIONS)) {
    warningText.classList.remove("hidden");
    warningText.textContent = "Format audio non supporté. Utilise MP3, WAV, M4A, AAC, OGG ou WEBM.";
    audioInput.value = "";
...
generateBtn.addEventListener("click", async () => {
  ...
  if (!isVideoOk) {
    showWarning(`La vidéo dépasse la limite autorisée. ${MAX_RECOMMENDED_SECONDS} secondes recommandées, ${MAX_ALLOWED_SECONDS} secondes tolérées maximum.`);
    return;
  }

  if (!isAudioOk) {
    showWarning(`L’audio dépasse la limite autorisée. ${MAX_RECOMMENDED_SECONDS} secondes recommandées, ${MAX_ALLOWED_SECONDS} secondes tolérées maximum.`);
    return;
  }
