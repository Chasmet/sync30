...
const VEED_RECOMMENDED_SECONDS = 30;
...
function getCurrentMaxSeconds() {
  ...
  return engine === "kling" ? KLING_MAX_SECONDS : VEED_MAX_SECONDS;
}
...
function getDurationErrorMessage(type) {
  ...
  return `Le ${type === "video" ? "fichier vidéo" : "fichier audio"} dépasse la limite autorisée. ${VEED_RECOMMENDED_SECONDS} secondes recommandées, ${VEED_MAX_SECONDS} secondes tolérées maximum.`;
}
...
function updateEngineTexts() {
  ...
  if (engine === "kling") {
    ...
    engineRulesText.textContent =
      "Mode VEED actif : 30 secondes recommandées, 32 secondes maximum pour la vidéo et l’audio.";
    engineDescription.textContent =
      "VEED = plus stable • 30 secondes recommandées, 32 secondes maximum.";
    videoRulesText.textContent =
      "Formats supportés : MP4, WEBM, MOV, M4V, OGG • VEED : 30 secondes recommandées • tolérance jusqu’à 32 secondes.";
    audioRulesText.textContent =
      "Formats supportés : MP3, WAV, M4A, AAC, OGG, WEBM • VEED : 30 secondes recommandées • tolérance jusqu’à 32 secondes.";
  }
}
