...
const LOCAL_USER_ID_KEY = "sync30_local_user_id";
...
function updateEngineTexts() {
  ...
  if (engine === "veed") {
    ...
    hintText.textContent = isAdmin
      ? "Ton mode admin est actif. Générations gratuites illimitées."
      : "Chaque navigateur a maintenant son propre espace vidéo local.";
  }
}
...
function updateAccountDisplay() {
  if (isAdmin) {
    ...
    accountBadge.textContent = "Espace privé local";
    hintText.textContent = "Chaque navigateur a maintenant son propre espace vidéo local.";
  }
}
    ...
    if (allVideos.length === 0) {
      videosContainer.innerHTML = `
        <div class="upload-box">
          ...
          <p>Les vidéos de cet espace local apparaîtront ici.</p>
        </div>
      `;
      return;
    }
