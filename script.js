...
let currentWalletSeconds = 0;
let detectedVideoDuration = null;
let detectedAudioDuration = null;
...
function getSelectedEngineBaseUrl() {
  return getSelectedEngine() === "veed" ? VEED_API_URL : SYNCUP_API_URL;
}

function getCurrentMaxSeconds() {
  return getSelectedEngine() === "veed" ? VEED_TOLERANCE_SECONDS : SYNCUP_MAX_SECONDS;
}
...
function formatDurationOneDecimal(value) {
  return `${Number(value || 0).toFixed(1).replace(".", ",")} s`;
}

function formatWholeSeconds(value) {
  return `${Math.max(0, Math.floor(Number(value || 0)))} s`;
}
...
function getEstimatedBilledSeconds() {
  if (!Number.isFinite(detectedVideoDuration) || detectedVideoDuration <= 0) {
    return 0;
  }
  return roundSecondsForBilling(detectedVideoDuration);
}

function updateEstimateCard() {
  const hasVideo = !!videoInput?.files?.[0] && Number.isFinite(detectedVideoDuration);
  ...
  const billedSeconds = getEstimatedBilledSeconds();
  const engine = getSelectedEngine();
  const modeLabelText = engine === "veed" ? "Qualité" : "Rapide";

  estimateModeText.textContent = modeLabelText;
  estimateDurationText.textContent = formatDurationOneDecimal(detectedVideoDuration);
  estimateCostText.textContent = formatWholeSeconds(billedSeconds);

  if (isAdminUser()) {
    estimateRemainingText.textContent = "Illimité";
  } else if (!currentUser) {
    estimateRemainingText.textContent = "Connecte-toi";
  } else {
    const remaining = currentWalletSeconds - billedSeconds;

    if (remaining >= 0) {
      estimateRemainingText.textContent = formatWholeSeconds(remaining);
    } else {
      estimateRemainingText.textContent = `Temps insuffisant (${formatWholeSeconds(currentWalletSeconds)} dispo)`;
    }
  }
...
function updateAccountDisplay() {
  ...
  if (currentUser) {
    ...
    creditsLabel.textContent = formatWholeSeconds(currentWalletSeconds);
    accountBadge.textContent = "Compte personnel";
    return;
  }

  modeLabel.textContent = "Utilisateur";
  creditsLabel.textContent = "0 s";
...
function updateHintText() {
  ...
  const modeText =
    engine === "veed"
      ? "Mode qualité sélectionné."
      : "Mode rapide sélectionné.";

  if (isAdminUser()) {
    hintText.textContent = `${modeText} Ton compte admin reste illimité.`;
    return;
  }
  ...
  if (currentWalletSeconds > 0) {
    hintText.textContent = `${modeText} Tu utilises uniquement le temps réellement consommé.`;
    return;
  }

  hintText.textContent = `${modeText} Ton temps disponible est à 0 s pour l’instant.`;
}
...
function updateEngineTexts() {
  ...
  if (engine === "veed") {
    engineRulesText.textContent =
      "Mode qualité actif : jusqu’à 30 secondes affichées, 32 secondes tolérées techniquement.";
    engineDescription.textContent =
      "VEED = rendu plus premium avec la même règle d’arrondi de temps.";
    ...
    engineRulesText.textContent =
      "Mode rapide actif : vidéo et audio limités à 9 secondes maximum.";
    engineDescription.textContent =
      "Sync-up = lip-sync économique et rapide.";
    videoRulesText.textContent =
...
async function maybeHandleRecoverySession(eventName) {
  ...
  const shouldHandle =
    eventName === "PASSWORD_RECOVERY" || urlLooksLikeRecoveryLink();
  ...
  if (!newPassword) {
    alert("Réinitialisation annulée. Redemande un nouveau mail si besoin.");
    return;
  }
...
async function fetchWallet() {
  ...
  try {
    const response = await fetch(`${getSelectedEngineBaseUrl()}/wallet`, {
      headers: {
    ...
    if (response.ok && data.ok) {
      currentWalletSeconds = Number(data.secondsBalance || 0);
    }
  } catch {}
...
async function readMediaDuration(file, type) {
  return new Promise((resolve, reject) => {
    ...
    element.onerror = () => {
      URL.revokeObjectURL(objectUrl);
      reject(new Error("Lecture du média impossible"));
    };
  });
}
...
async function revalidateSelectedFiles() {
  const maxSeconds = getCurrentMaxSeconds();

  if (Number.isFinite(detectedVideoDuration) && detectedVideoDuration > maxSeconds) {
    showWarning(getDurationErrorMessage("video"));
    clearSelectedVideo();
    return;
  }

  if (Number.isFinite(detectedAudioDuration) && detectedAudioDuration > maxSeconds) {
...
async function playVideoInsideApp(playUrl, downloadUrl, fileName) {
  try {
    clearWarning();
    await setResultVideoSecure(playUrl, downloadUrl, fileName);

    hintText.textContent = "Lecture chargée dans l’application. Télécharge vite ta vidéo, le stockage est temporaire.";
    ...
    showWarning(`Erreur lecture dans l'application : ${error.message}`);
  }
}
...
function updateWalletFromResponse(data) {
  if (isAdminUser()) {
    updateAccountDisplay();
    updateHintText();
    updateEstimateCard();
  ...
  if (typeof data?.secondsBalance === "number") {
    currentWalletSeconds = data.secondsBalance;
    updateAccountDisplay();
    updateHintText();
    updateEstimateCard();
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    syncupPollingInterval = setInterval(async () => {
      ...
      try {
        ...
        updateWalletFromResponse(data);

        if (data.status === "starting") {
          progressText.textContent = "Sync-up démarre...";
          return;
        }

        if (data.status === "processing") {
          progressText.textContent = "Sync-up traite la vidéo...";
          return;
        }
        ...
        if (data.status === "succeeded") {
          ...
          hintText.textContent = "Sync-up terminé. Maximum 3 vidéos par compte. Télécharge rapidement.";
...
async function loadVideos() {
  ...
  try {
    ...
    if (allVideos.length === 0) {
      videosContainer.innerHTML = `
        <div class="upload-box">
          ...
          <p>Les vidéos de ton compte apparaîtront ici. Maximum 3 vidéos conservées.</p>
        </div>
      `;
      return;
    }
    ...
    allVideos.forEach((item) => {
      ...
      helper.textContent = "Regarde ici dans l’application, ou utilise Ouvrir en secours. Les anciennes vidéos sont supprimées automatiquement.";

      const watchHereButton = document.createElement("button");
      watchHereButton.className = "btn btn-primary";
      watchHereButton.type = "button";
      watchHereButton.textContent = "Regarder ici";
      watchHereButton.addEventListener("click", async () => {
        await playVideoInsideApp(item.playUrl, item.downloadUrl, item.name || "video.mp4");
      });

      const openButton = document.createElement("button");
      openButton.className = "btn btn-soft btn-inline";
      ...
      mainActions.className = "video-actions-main";
      mainActions.appendChild(watchHereButton);

      const secondaryActions = document.createElement("div");
      secondaryActions.className = "video-actions-inline";
      secondaryActions.appendChild(openButton);
      secondaryActions.appendChild(downloadButton);
      secondaryActions.appendChild(deleteButton);

      mainActions.appendChild(secondaryActions);
...
videoInput?.addEventListener("change", async () => {
  ...
  try {
    ...
    detectedVideoDuration = duration;
    videoFileName.textContent = `${file.name} • ${formatDurationOneDecimal(duration)}`;
    clearWarning();
    updateEstimateCard();
  } catch (error) {
    showWarning(`Impossible de lire la durée de la vidéo : ${error.message}`);
    clearSelectedVideo();
  }
});
...
audioInput?.addEventListener("change", async () => {
  ...
  try {
    ...
    detectedAudioDuration = duration;
    audioFileName.textContent = `${file.name} • ${formatDurationOneDecimal(duration)}`;
    clearWarning();
  } catch (error) {
    showWarning(`Impossible de lire la durée de l’audio : ${error.message}`);
    clearSelectedAudio();
  }
});
...
generateBtn?.addEventListener("click", async () => {
  ...
  if (!Number.isFinite(detectedVideoDuration) || detectedVideoDuration <= 0) {
    showWarning("Impossible de déterminer la durée de la vidéo.");
    return;
  }
  ...
  if (!isAdminUser()) {
    if (currentWalletSeconds < billedSeconds) {
      showWarning(`Temps insuffisant. Il faut ${formatWholeSeconds(billedSeconds)} et tu as ${formatWholeSeconds(currentWalletSeconds)}.`);
      return;
    }
  }
  ...
  try {
    ...
    if (!isAdminUser()) {
      formData.append("duration_seconds", String(detectedVideoDuration));
    }
    ...
    if (engine === "veed") {
      ...
      hintText.textContent = "VEED terminé. Maximum 3 vidéos par compte. Télécharge rapidement.";
    ...
    hintText.textContent = "Sync-up traite la vidéo. En cas d’affluence, le traitement peut être un peu plus long.";
...
async function initApp() {
  ...
  if (urlLooksLikeRecoveryLink()) {
    await maybeHandleRecoverySession("PASSWORD_RECOVERY");
  }
}
