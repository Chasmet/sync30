...
function getDurationErrorMessage(type) {
  ...
  return `Le ${type === "video" ? "fichier vidéo" : "fichier audio"} dépasse la limite de VEED. Maximum autorisé : ${VEED_MAX_SECONDS} secondes.`;
}
...
function updateEngineTexts() {
  ...
  if (engine === "kling") {
    ...
    engineRulesText.textContent =
      "Mode VEED actif : vidéo et audio limités à 30 secondes maximum.";
    engineDescription.textContent =
      "VEED = plus stable • 30 secondes maximum.";
    videoRulesText.textContent =
      "Formats supportés : MP4, WEBM, MOV, M4V, OGG • VEED : 30 secondes maximum.";
    audioRulesText.textContent =
      "Formats supportés : MP3, WAV, M4A, AAC, OGG, WEBM • VEED : 30 secondes maximum.";
  }
}
