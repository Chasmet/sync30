...
generateBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (engine === "veed") {
      ...
      const finalUrl = buildPlayableUrl(data.playUrl || data.videoUrl);
      setResultVideo(finalUrl, data.downloadUrl || data.videoUrl, "sync30-veed.mp4");
    ...
    hintText.textContent =
      "Sync-up traite la vidéo. La vidéo apparaîtra dès qu’elle sera prête.";
...
function setResultVideo(playUrl, downloadUrl, fileName) {
  const finalPlayUrl = buildPlayableUrl(playUrl);
  const finalDownloadUrl = downloadUrl || playUrl;

  previewVideo.pause();
  previewVideo.removeAttribute("src");
  previewVideo.load();

  previewVideo.src = finalPlayUrl;
  previewVideo.setAttribute("playsinline", "true");
  previewVideo.setAttribute("preload", "metadata");
  previewVideo.load();

  downloadBtn.href = finalDownloadUrl;
  downloadBtn.download = fileName || "sync30-result.mp4";
    ...
    syncupPollingInterval = setInterval(async () => {
      ...
      try {
        const response = await fetch(
          `${SYNCUP_API_URL}/sync-status/${jobId}`,
          {
            headers: {
              "x-user-id": userId
            }
        ...
        if (data.status === "succeeded") {
          ...
          hintText.textContent =
            "Sync-up terminé. La vidéo est maintenant prête.";
...
async function loadVideos() {
  ...
  try {
    ...
    allVideos.forEach((item) => {
      ...
      video.src = buildPlayableUrl(item.playUrl);
      ...
      openLink.textContent = "Ouvrir la vidéo";
